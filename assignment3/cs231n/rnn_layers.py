...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  scores = prev_h.dot(Wh) + x.dot(Wx) + b
  next_h = np.tanh(scores)
  cache = (scores, Wx, Wh, x, prev_h.copy())
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  #Unroll the cache
  scores, Wx, Wh, x, prev_h = cache

  ##The derivative of scores is computed with respect to the weight matrix
  dscores = dnext_h * (1 - np.tanh(scores)**2 )

  #The derivative of the data weights is simply the backward matrix
...
def lstm_backward(dh, cache):
  ...
  for t in xrange(T-1, -1, -1):
      ...
      #Updating the gradient on LSTM
      dx[:, t, :], dprev, dprev_c, dWx_temp, dWh_temp, db_temp = lstm_step_backward(dh[:, t, :]+dprev, dprev_c, cache[t])
      dWx += dWx_temp
      dWh += dWh_temp
      db  += db_temp
